...
*/
    ...
    */
    ...
    UBApplication app("takhte noandishan", argc, argv);

    // === بخش فعال‌سازی زبان فارسی و راست‌چین ===
    QTranslator translator;
    QLocale locale;
    if (locale.language() == QLocale::Persian) {
        if (translator.load(":/i18n/OpenBoard_fa.qm")) {
    ...
    if (args.size() > 1) { // Changed condition to > 1 as the first arg is the app name
    ...
    int result = 0;
    if (app.isPrimaryInstance()) // Corrected method name
    {
