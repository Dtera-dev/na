...
APPLICATION_NAME="noandishanboard"
...
function notify {
    ...
    if [ -x "$GROWLNOTIFY" ]; then
        $GROWLNOTIFY --name noandishanboard-build --iconpath /Developer/Applications/Xcode.app --message "$1" "noandishanboard"
    fi
...
echo $(2>&1 xcrun notarytool submit --wait --apple-id "$USER" --team-id "$TEAMID" noandishanboard.dmg)
