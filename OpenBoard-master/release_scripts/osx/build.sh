...
function notify {
    ...
    if [ -x "$GROWLNOTIFY" ]; then
        $GROWLNOTIFY --name noandishan board-build --iconpath /Developer/Applications/Xcode.app --message "$1" "noandishan board"
    fi
...
do
    ...
    do
        # looping through the noandishan board availables languages
        ...
        if [ ! -z $directoryLanguageCode ]; then
            if [[ $eachDirectory == *".lproj"* && $eachDirectory != "empty.lproj" && $directoryLanguageCode == *$languageCode* ]]; then
                # noandishan board translation found for qt translation file
...
trap "defaults write org.oe-f.noandishan board.release Running -bool NO" EXIT

notify "Running noandishan board release script (`date`)"

cd $PROJECT_ROOT

script_is_running=`defaults read org.oe-f.noandishan board.release Running 2>/dev/null`
if [[ $? -eq 0 ]] && [[ "$script_is_running" = "1" ]]; then
    trap EXIT
    abort "another release script already running"
fi
defaults write org.oe-f.noandishan board.release Running -bool YES
...
notify "If so, you can directly package noandishan board using the script 'package.sh'"
